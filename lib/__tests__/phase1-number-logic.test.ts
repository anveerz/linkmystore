import { FREE_PRODUCT_LIMIT, PLAN_FEATURES, PRO_PLAN_PRICE, calculatePlatformFee } from '@/lib/constants'

describe('phase 1 numeric rules', () => {
  it('keeps free tier product cap at 5 and pro pricing at INR 199', () => {
    expect(FREE_PRODUCT_LIMIT).toBe(5)
    expect(PLAN_FEATURES.free.maxProducts).toBe(5)
    expect(PRO_PLAN_PRICE).toBe(19900)
  })
