...
export async function POST(request: Request) {
  try {
    ...
    const filePath = `customization/${new Date().toISOString().slice(0, 10)}/${randomUUID()}.${ext}`
    ...
    const { data: publicUrlData } = admin.storage.from('product-images').getPublicUrl(filePath)
